...
>>>>>>> 49d75a4e1914e1d8a594f1974410f33c390a2857
              </div>
              <div className="new-font">
                <h5>{ands.auctionTitle}</h5>
                <p
                  className={
                    calculateTimeDifference(ands.auctionEndDate).hours < 8
                      ? "p-time-short"
                      : ""
                  }
                >
                  {calculateTimeDifference(ands.auctionEndDate).days > 0
                    ? `남은 시간: ${
                        calculateTimeDifference(ands.auctionEndDate).days
                      }일`
                    : calculateTimeDifference(ands.auctionEndDate).hours >= 0
                    ? `남은 시간: ${
                        calculateTimeDifference(ands.auctionEndDate).hours < 10
                          ? "0"
                          : ""
                      }${calculateTimeDifference(ands.auctionEndDate).hours}:${
                        calculateTimeDifference(ands.auctionEndDate).minutes <
                        10
                          ? "0"
                          : ""
                      }${
                        ...
                        calculateTimeDifference(ands.auctionEndDate).seconds <
                        10
                          ? "0"
                          : ""
                      }${calculateTimeDifference(ands.auctionEndDate).seconds}`
        ...
        <Modal>
          <div className="content">
            ...
            <div className="flex-row">
              <p className="times">
                {calculateTimeDifference(selectedItem.auctionEndDate).days > 0
                  ? `남은 시간: ${
                      calculateTimeDifference(selectedItem.auctionEndDate).days
                    }일`
                  : calculateTimeDifference(selectedItem.auctionEndDate)
                      .hours >= 0
                  ? `남은 시간: ${
                      calculateTimeDifference(selectedItem.auctionEndDate)
                        .hours < 10
                      ...
                      calculateTimeDifference(selectedItem.auctionEndDate).hours
                    }:${
                      calculateTimeDifference(selectedItem.auctionEndDate)
                        .minutes < 10
                        ? "0"
